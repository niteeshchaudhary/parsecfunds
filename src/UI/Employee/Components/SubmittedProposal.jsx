...
function SubmittedProposal() {
  ...
  function getfile() {
    const downloadTask = storage
      .ref(
        `file/${loadJob?.from || job?.from}/${loadJob?.job_id || job.job_id}/${
          auth.currentUser.email
        }/${loadProposal?.data?.documentPath || proposal?.documentPath}`
      )
      .getDownloadURL();
    downloadTask
      .then((dat) => {
  ...
  const editProposal = () => {
    if (job?.job_type == "fixed") {
      navigate("/SubmitProposalbyfixed", { state: state });
    } else if (loadJob?.job_type == "fixed") {
      const info = state || {};
      info["job"] = loadJob;
      info["proposal"] = loadProposal.data;
      navigate("/SubmitProposalbyfixed", { state: info });
    }
    if (job?.job_type == "hourly") {
      navigate("/SubmitProposals", { state: state });
    } else if (loadJob?.job_type == "hourly") {
      const info = state || {};
      info["job"] = loadJob;
      info["proposal"] = loadProposal.data;
      navigate("/SubmitProposals", { state: info });
    }
  };
    ...
    <>
      {proposal ? (
        ...
        >
          ...
          />
          <p
            style={{ fontSize: "1rem", color: "#006872", fontWeight: "600" }}
            className=""
          >
      ...
      <div className="jobdetail-box">
        ...
        >
          <button
            type="submit"
            onClick={() => navigate("/LatestProposal")}
          ...
          {status == "submit" || status == "new" ? (
            <>
              ...
              {loadingState && (
                <SpinnerDotted
                  style={{ height: "2rem", marginLeft: "10rem" }}
                />
              )}
              {!loadingState && (
                <button
