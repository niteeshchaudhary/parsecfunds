...
const postJob = httpsCallable(functions, "postJob");
...
function Postjob() {
  ...
  const validate = (values) => {
    ...
    var word_length_job_detail = values.job_detail.trim().split(/[\s]+/).length;
    ...
    if (!values.yearexperience) {
      errors.yearexperience = "*Year of experience is required!";
    }
    if (!regex.test(values.Question)) {
      errors.Question = "*This is not a valid Question format!";
    } else if (values.Question.length > 100) {
      errors.Question = "*Question cannot exceed more than 100 characters";
    }
    if (!values.role) {
    ...
    } else if (fixed && !regex_num.test(values.fixed)) {
      errors.fixed = "*Value can only be numeric!";
    } else if (fixed && values.fixed > 50000) {
    ...
    if (!values.job_detail) {
      errors.job_detail = "*job details is required";
    } else if (!regex_spec.test(values.job_detail)) {
      errors.job_detail = "*This is not a valid Job detail format!";
    } else if (word_length_job_detail < 10) {
      errors.job_detail = "*job details must be more than 10 words";
    } else if (values.job_detail.length > 1500) {
      errors.job_detail =
        "*Job details cannot exceed more than 1500 characters";
    }
  ...
  function toOther() {
    ...
    const data = {
      ...
      details: document.getElementById("job_detail").value,
        ...
        <div className="container-job" style={{ width: "57.6vw" }}>
          ...
          <div className="mid-input-wrap">
            ...
            >
              ...
              {projectlevel.map((lvl, index) => (
                <option value={lvl.val()} key={"pro" + index}>
                  {lvl.val()}
                </option>
              ))}
            </select>
            ...
            >
              ...
              {jobduration.map((duration, index) => (
                <option value={duration.val()} key={"jb" + index}>
                  {duration.val()}
                </option>
              ))}
            </select>
          ...
          <div className="mid-input-wrap">
            ...
            >
              ...
              {yoe.map((ye, index) => (
                <option value={ye.val()} key={"ye" + index}>
                  {ye.val()}
                </option>
              ))}
            </select>
